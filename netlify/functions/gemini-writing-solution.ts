...
function buildSolutionInstruction(task: 1 | 2, targetLevel: string) {
  const minWords = task === 1 ? 150 : 250
  return `You are an IELTS Writing tutor. Write a complete model answer for IELTS Writing Task ${task}.

The answer must target ${targetLevel} competence (not native C2 level). Use clear, natural English appropriate for that level.

${
  task === 1
    ? 'When an image of a chart, map, diagram, or process is provided, base your answer on that visual together with the printed prompt.'
    : ''
}

Requirements:
- Write only the model answer text (no title, no commentary, no markdown fences).
- Aim for at least ${String(minWords)} words, typical for IELTS Task ${String(task)}.
- Use appropriate paragraph breaks (blank lines between paragraphs).
- Do not mention that you are an AI or that this is a model answer.`
}
...
function parseGeminiRetryMaxAttempts(override?: number) {
...
export const handler: Handler = async (event, context) => {
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction: buildSolutionInstruction(input.task, input.targetLevel),
  })
...
Write the model answer now.`
  ...
  try {
    ...
    if (input.task === 1 && imageUrl !== null) {
      ...
      parts.push({
        text: 'The image below is the Task 1 visual (chart, map, diagram, or process) to describe. Use it together with the printed prompt.',
      })
    ...
    return {
      statusCode: 200,
      headers: jsonHeaders,
      body: JSON.stringify({ solution, modelName }),
    }
  } catch (err) {
    ...
    if (message.includes('timed out') || message.includes('no time left before function timeout')) {
      message = `${message} Mitigations: raise the synchronous function timeout in the Netlify UI (then redeploy) and ensure netlify.toml matches your plan; netlify dev often simulates ~30s unless you run netlify link; try a faster Gemini model if responses are slow.`
    }
    if (
      ...
      message = `${message} The API may be temporarily overloaded; this function retries automatically up to ${String(parseGeminiRetryMaxAttempts())} time(s). Try again in a minute.`
    }
    return {
      statusCode: 502,
      headers: jsonHeaders,
